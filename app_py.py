...
st.title("🌍 Land Classification – Desert vs Agriculture (U-Net)")
...
try:
    ...
    st.error("❌ Could not load model_unet.h5 — please upload it to the same folder.")
    st.stop()
...
# ------------------------------------------------------
# (5) If user uploads an image → Show prediction
# ------------------------------------------------------
...
if uploaded_file is not None:
    ...
    if prop_agri > 0.05:
        st.success(f"🌱 Suitable for agriculture — {prop_agri*100:.2f}% green area detected")
    else:
        st.warning(f"🏜️ Mostly desert — {prop_agri*100:.2f}% green area detected")
